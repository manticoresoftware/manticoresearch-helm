...
To assign a [Kubernetes VolumeAttributesClass](https://kubernetes.io/docs/concepts/storage/volume-attributes-classes/) to worker PVCs, set `worker.persistence.volumeAttributesClassName`. This is useful for CSI drivers that implement `ModifyVolume`, for example to change provisioned IOPS or throughput. The class must already exist in the cluster; leave the value empty to omit the field. VolumeAttributesClass is generally available in Kubernetes 1.34 and later.
...
Wordforms can be provided from Kubernetes ConfigMaps or from any extra volume mounted into worker pods.

Create a ConfigMap from local wordforms files:

```bash
kubectl create configmap product-wordforms \
  --from-file=products.txt=./wordforms/products.txt
```

Mount it into workers:

```yaml
worker:
  wordforms:
    enabled: true
    mountPath: /usr/share/manticore/wordforms
    configMaps:
      - name: product-wordforms
        items:
          - key: products.txt
            path: products/products.txt
```

Use the mounted file in table settings:

```sql
CREATE TABLE products(title text) wordforms='/usr/share/manticore/wordforms/products/products.txt';
```

For multiple tables, create separate ConfigMaps or use separate keys/paths:
...
worker:
  wordforms:
    ...
    configMaps:
      - name: product-wordforms
      - name: article-wordforms
        items:
          - key: en.txt
            path: articles/en.txt
```

For large wordforms, use a PVC or another Kubernetes volume:
    ...
    extraVolumes:
      - name: wordforms-pvc
        persistentVolumeClaim:
          claimName: manticore-wordforms
    extraVolumeMounts:
      - name: wordforms-pvc
        mountPath: /usr/share/manticore/wordforms-large
        readOnly: true
```
...
| balancer.podLabels | Additional labels added only to balancer pods. Merged with `podLabels`. | {} |
...
| worker.podLabels | Additional labels added only to worker pods. Merged with `podLabels`. Useful for external traffic selectors such as `appgroup: manticore-indexes`. | {} |
| worker.replicationMode | Workers replication mode                                                                                                  | master-slave                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| worker.clusterName | Name of replication cluster                                                                                               | manticore                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| worker.searchd.extraArgs | Extra command-line arguments appended to the worker `searchd` process. Only long-form flags are allowed: `--flag` or `--flag=value` | []                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| worker.persistence.size | Worker max storage size (mounted volume)                                                                                  | 1Gi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| worker.persistence.storageClass | Worker storage class                                                                                                      | -                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| worker.persistence.volumeAttributesClassName | Optional Kubernetes VolumeAttributesClass assigned to worker PVCs. Requires a compatible CSI driver. | unset |
...
| worker.wordforms.enabled | Mount wordforms into worker pods                                                                                         | false                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| worker.wordforms.mountPath | Mount path for ConfigMap-backed wordforms                                                                               | /usr/share/manticore/wordforms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| worker.wordforms.configMaps | User-created ConfigMaps to project under `worker.wordforms.mountPath`                                                  | []                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| worker.wordforms.extraVolumes | Extra worker volumes for large wordforms                                                                               | []                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| worker.wordforms.extraVolumeMounts | Extra worker volume mounts for large wordforms                                                                    | []                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| worker.mlock.enabled                 | Adds `CAP_IPC_LOCK` to the worker container. Enable this when `worker.config.content` uses `access_* = mlock`, otherwise Kubernetes may allow only a very small locked-memory limit and Manticore can log `mlock() failed: Cannot allocate memory` even when the pod has enough RAM. | false                                                                                              |
| worker.mlock.validate                | Fails Helm rendering when `worker.config.content` uses `access_* = mlock` but neither `worker.mlock.enabled` nor `securityContext.capabilities.add: [IPC_LOCK]` is configured. | true                                                                                               |
...
| podLabels                            | Additional labels added to worker and balancer pods. Chart selector labels cannot be overridden. Use `worker.podLabels` or `balancer.podLabels` for component-specific labels. | {}                                                                                                 |
| podSecurityContext                   | Allows to set pod security contexts                                                                 | {}                                                                                                 |
| securityContext                      | Allows to set security contexts (see k8s docs for details). Add `capabilities.add: [IPC_LOCK]`, or set `worker.mlock.enabled=true`, when Manticore uses `access_* = mlock`. | {}                                                                                                 |
